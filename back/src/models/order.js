...
function hourToLetter(hour) {
  return "ABCDEFGHIJKLMNOPQRSTUVWXYZ"[hour] || "Z";
}

async function generateOrderCode(date) {
  const now = date instanceof Date ? date : new Date();
  const yyyymmdd = now.toISOString().slice(0, 10).replace(/-/g, "");
  const hourLetter = hourToLetter(now.getHours());
...
const Order = sequelize.define(
  "Order",
  {
    ...
    order_code: {
      ...
      unique: true,
      validate: {
        notEmpty: { msg: "Código do pedido não pode estar vazio" },
      },
    },
...
function getPaymentStatus(order) {
  return order.paid ? "efetuado" : "pendente";
}
...
async function markAsPending(order) {
  order.paid = false;
  return await order.save();
}

async function findPaid() {
  return await Order.findAll({ where: { paid: true, active: true } });
}

async function findPending() {
  return await Order.findAll({ where: { paid: false, active: true } });
}
